...
void objectTransport(int scanx, int scany, float &yTogo, float &xTogo, bool &isThere, int row, int powerx, int color)
{
...
void reset(float scanx, float scany)
{
	nMotorEncoder[motorA] = nMotorEncoder[motorB] = 0;
	while(abs(nMotorEncoder[motorB]) < scany)
	{
		motor[motorB] = 20;
	}
	...
	nMotorEncoder[motorA] = 0;
	while(abs(nMotorEncoder[motorA]) < scanx)
	{
		motor[motorA] = -20;
	}
	motor[motorA] = 0;
...
task main()
{
	...
	while (colourChosenNum != 404 && plays == true)
	{
		...
		if (race == false)
		{
			...
			objectTransport(scanx, scany, yTogo, xTogo, isThere, row, powerx, colourChosenNum);
			eraseDisplay();
			displayString(8, "Press touch sensor to end");
